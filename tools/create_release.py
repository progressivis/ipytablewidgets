...
@click.option('--release-type', '-r', required=True,
              type=click.Choice(['major', 'minor', 'patch', 'manual'],
...
def release_it(skip_pypi, skip_npm_publish, release_type):
    ...
    if not skip_pypi:
        ...
        if not osp.exists(pjoin(home_dir, '.pypirc')):
            inp = input('PyPI config file .pypirc does not exist in your homedir.'
                        'Continue anyway [y/n]?')
    ...
    if vers in existing_tags:
        inp = input(f"Release {vers} already exists. Continue with packaging anyway [y/n]?")
        if inp != 'y':
            print("Exiting ...")
            return
    else:
    ...
    https_url = repo.remotes.origin.url.replace(':','/', 1).replace('git@', 'https://') # TO DO: improve
    print(https_url)
    if skip_pypi and skip_npm_publish:
        return
    pack_name = subprocess.check_output([sys.executable, 'setup.py', '--name'], cwd=repo_root).decode('utf8').strip()
    ...
    subprocess.check_call(
        [sys.executable, '-m', 'build'], cwd=repo_dir)
    if not skip_pypi:
